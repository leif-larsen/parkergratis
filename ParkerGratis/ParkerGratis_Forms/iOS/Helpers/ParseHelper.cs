...
[assembly: Xamarin.Forms.Dependency (typeof(ParkerGratis_Forms.iOS.Helpers.ParseHelper))]
namespace ParkerGratis_Forms.iOS.Helpers
{
	public class ParseHelper : IParse
	{
		private List<ParkingInfo> _geoList = new List<ParkingInfo>();

		public async Task<bool> addNewParking(string name, double lat, double longitude, string type_other, int type, string extraInfo = "", string address = "") 
		{
			try {
		...
		public async Task<List<ParkingInfo>> execGeoQuery(double userLat, double userLong, double distance) 
		{
			...
			query = query.WhereWithinDistance ("location", userGeoPoint, new ParseGeoDistance (distance / 6371))
				.WhereNotEqualTo ("activated", false);
		...
		public async Task<bool> reportParkingSpot(string objId) 
		{
			try{
				...
				var nrOfReported = parseObj.Get<int>("nrOfReported");

				if(nrOfReported >= 3)
					parseObj["activated"] = false;
